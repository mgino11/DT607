...
**Because it is a sample of population the distribution should be somewhat similar not identical. That means we could expect values close to 80% and 20% respectively.**
```{r - samp1-dist}
...
**Since we are selecting a random sample from the population the sample proportion should not match (odds are very low) in this case the proportions should be somewhat different NOT very different because both samples are coming from the same population**
...
**The sample that is closest to the population is the one that will provide a more accurate estimate of the population**
...
The centers of the distributions are always at the population proportion p
The sample proportion $\hat{p}$ is always **unbiased** when the data are independent
$\hat{p}$ is always centered at the population parameter p
...
sample_props_small <- global_monitor %>%
                    ...
                    count(scientist_work) %>%
                    mutate(p_hat = n /sum(n)) %>%
                    filter(scientist_work == "Doesnt Benefit")
...
ggplot(data = sample_props15 , aes(x = p_hat)) +
  ...
  labs(
    x = "p_hat (Benefits)",
    title = "Sampling distribution of people who believe scientist work Benefits their    
    lives",
